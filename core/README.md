...
## v1.6.72
1. `RepVideoPlayUrl.urls` 增加 `subtitleUrl` 字段，用于传递字幕链接。
2. `UZHome` 推荐扩展 增加 `getSearchSuggest` 方法，用于获取搜索建议。
...
4. 返回视频播放对象 urls 支持传递 `audioUrl` 字段，用于传递音频链接，支持 `platformUrl` 传递视频平台链接
